          ...
          {NAV_ITEMS.map((item) => (
            <button
              ...
              className={`whitespace-nowrap rounded-full px-3 py-2 text-xs font-semibold transition ${
                activeNav === item ? "bg-[var(--accent)] text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
              }`}
            >
              {item}
            </button>
      ...
      <div className="grid gap-4 xl:grid-cols-[240px_minmax(0,1fr)]">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3 p-3 md:hidden">
            {displayed.map((t) => (
              <Card key={t.id} className="p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${STATUS_STYLES[t.status]}`}>{t.status}</span>
                </div>
                <div className="mt-3 grid grid-cols-1 gap-2">
                  <MobileMeta label="Priority" value={t.priority} />
                  <MobileMeta label="Category" value={t.category} />
                  <MobileMeta label="Department" value={t.dept} />
                  <MobileMeta label="Assignee" value={t.assignee} />
                  <MobileMeta label="Age" value={t.age} />
                </div>
            ...
            {displayed.length === 0 && <div className="px-5 py-10 text-center text-sm text-slate-400">No tickets in this view.</div>}
          </div>
