    ...
    <div className="space-y-5 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <Card key={item.label} className="flex items-center gap-3 px-4 py-4 sm:px-5">
              ...
              <div key={record.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{formatAttendanceDate(record.attendanceDate)}</p>
                  </div>
                  <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${statusBadgeClass(record.status)}`}>
                    {record.status}
                  </span>
                </div>
                <div className="mt-3 space-y-2">
                  <MobileMeta label="Department" value={departmentLabel(record.department)} />
                  <MobileMeta label="Unit" value={record.unit ?? "—"} />
                  <MobileMeta label="Clock In" value={formatAttendanceClockTime(record.clockInAt)} />
                  <MobileMeta label="Clock Out" value={formatAttendanceClockTime(record.clockOutAt)} />
                  <MobileMeta label="Hours" value={`${record.hours.toFixed(1)}h`} />
                </div>
              </div>
            ))
          )}
