...
export function RotaBuilder({ unitName }: { unitName: string }) {
  ...
  const unitStaff = staff.filter(
    (s) => s.department === "Non-Clinical Staff" && s.unit === unitName && s.status === "Active",
  );
  ...
  const loadShifts = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchShiftsForUnit(unitName, weekDates[0], weekEnd);
      setShifts(data);
    } finally {
      setLoading(false);
    }
  }, [unitName, weekDates[0], weekEnd]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function handleAssign() {
    ...
    setSaving(true);
    try {
      // Remove existing shift for this cell first
      ...
      const created = await createNcShift({
                ...
                {weekDates.map((date, i) => {
                  ...
                  return (
                    <th
                      key={date}
                      className={cn(
                        "min-w-[88px] px-2 py-3 text-center text-xs font-semibold uppercase tracking-wide",
                        isToday ? "text-[var(--accent)]" : "text-slate-500",
                      )}
                    >
              ...
              {unitStaff.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-6 py-10 text-center text-sm text-slate-400">
                    No active staff in <strong>{unitName}</strong> yet.
                    Assign staff to this unit from the HR Staff Directory.
                  </td>
                </tr>
              ) : (
                unitStaff.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="sticky left-0 z-10 bg-white px-4 py-2.5">
                      <div className="flex items-center gap-2.5">
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-lime-100 text-xs font-bold text-lime-700">
                          {getInitials(s.name)}
                        </div>
                        ...
                        >
                          {shift && style ? (
                            <button
                              ...
                              onClick={() => {
                                setCell({ staffId: s.id, date });
                                setPickedType(shift.shiftType);
                              }}
                              onContextMenu={(e) => {
                                e.preventDefault();
                              ...
                              className={cn(
                                "inline-flex flex-col items-center rounded-lg px-2 py-1 text-[11px] font-bold transition hover:opacity-80",
            ...
            <div className="space-y-2 px-5 py-4">
              ...
              {SHIFT_TYPES.map((t) => (
                <button
                  key={t.value}
                  type="button"
                  onClick={() => setPickedType(t.value)}
                  className={cn(
                    "flex w-full items-center justify-between rounded-xl border px-4 py-3 text-left transition",
                    pickedType === t.value
                      ? `border-[var(--accent)]/40 ${t.bg}`
                      : "border-slate-200 hover:bg-slate-50",
                  )}
                >
                  <div>
                    <span className={cn("text-sm font-semibold", pickedType === t.value ? t.color : "text-slate-700")}>
                      {t.label}
                    </span>
          ...
          <div className="w-full max-w-sm rounded-2xl bg-white shadow-xl">
            <div className="px-5 py-5">
              ...
              <p className="mt-1 text-sm text-slate-500">
                ...
                <strong>{shiftStyle(confirmDelete.shiftType).label}</strong> shift on{" "}
                <strong>
                  {new Date(confirmDelete.shiftDate + "T00:00:00").toLocaleDateString("en-GB", {
                    weekday: "long", day: "numeric", month: "long",
                  })}
                </strong>
                .
